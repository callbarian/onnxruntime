...
static void RunMultiHeadAttentionTestEnv(
  ...
  // Enable all fused kernels (default setting)
  {
    // Unfused kernel only
    {
...
TEST(MultiHeadAttentionTest, CrossAttention_Batch2_HeadSize40) {
  ...
  GetCrossAttentionData(data);

  bool use_float16 = false;
  RunMultiHeadAttentionTestEnv(
      data.query_data, data.key_data, data.value_data, data.bias_data, data.key_padding_mask_data, data.mask_type, data.output_data,
      data.num_heads, data.batch_size, data.sequence_length, data.kv_sequence_length, data.hidden_size, data.v_hidden_size,
      use_float16);
}
